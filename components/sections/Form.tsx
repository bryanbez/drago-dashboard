...
import { validateTextboxInput } from "@/app/lib/utils/textboxValidator";
...
function WalletAddressFormInput() {
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full max-w-2xl">
      <form onSubmit={handleFormInput} className="flex flex-col w-full gap-2">
        <div className="flex items-center space-x-4 w-full">
          ...
          <button
            type="submit"
            disabled={!validateInput.success}
            className={`px-4 py-2 cursor-pointer font-semibold rounded-lg shadow-md focus:outline-none focus:ring-2 focus:ring-opacity-75 transition duration-150 ease-in-out ${
              status === "loading"
                ? "bg-gray-400 cursor-not-allowed"
                : "bg-blue-600 text-white focus:ring-blue-500"
            } ${
              !validateInput.success ? "bg-gray-400 cursor-not-allowed" : ""
            }`}>
            {status === "loading" ? (
              <svg
        ...
        <div>
          {validateInput.success ? (
            ...
            <p className="text-red-500 text-sm">
              {validateInput.error.issues[0]?.message}
            </p>
          )}
        </div>
      </form>
